...
impl PrometheusExporter {
    ...
    pub fn observe_genai_slo(&mut self, service: &str, evaluation: &GenAiSloEvaluation) {
        ...
        self.observe_mode_bool(
            ...
            "GenAI SLO throughput objective pass flag",
        );
        self.observe_mode_bool(
            "genai",
            service,
            ...
            "GenAI SLO TTFT objective pass flag",
        );
        self.observe_mode_bool(
            "genai",
            service,
    ...
    /// Record composite graph global and per-service metrics.
    ...
    /// Record web API report metrics for a specific service.
    pub fn observe_web_api_report(&mut self, service: &str, report: &WebApiSloReport) {
        self.observe_mode_gauge(
            "web_api",
            service,
            "availability",
            report.availability,
            "Web API availability ratio",
        );
        self.observe_mode_gauge(
            "web_api",
            service,
            "latency_sli",
            report.latency_sli,
            "Web API latency SLI ratio",
        );
        self.observe_mode_gauge(
            "web_api",
            service,
            "error_budget_seconds",
            report.error_budget_seconds,
            "Web API error budget in seconds",
        );
        self.observe_mode_gauge(
            "web_api",
            service,
            "burn_rate_5m",
            report.burn_rate_5m,
            "Web API burn rate over 5 minutes",
        );
        self.observe_mode_gauge(
            "web_api",
            service,
            "burn_rate_1h",
            report.burn_rate_1h,
            "Web API burn rate over 1 hour",
        );
    }

    /// Record error budget metrics for a service.
    ...
    fn observe_composite_service(&mut self, graph: &str, service: &CompositeServiceSloEvaluation) {
        let labels = vec![
        ...
        self.observe_gauge(
            ...
            "Composite service local score",
            service.local_score,
            labels.clone(),
        );
        self.observe_gauge(
            "composite_service_effective_score",
            "Composite service effective score",
            service.effective_score,
            labels.clone(),
        );
        self.observe_gauge(
            "composite_service_pass",
            "Composite service pass flag",
            bool_as_f64(service.pass),
            labels.clone(),
        );
        self.observe_gauge(
            "composite_service_dependency_adjusted",
            "Composite service dependency adjustment flag",
            bool_as_f64(service.dependency_adjusted),
            labels,
        );
    }
